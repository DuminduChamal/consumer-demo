...
and `producer-demo`). `pom.xml`'s `exec-maven-plugin` `mainClass` entries
were updated to match (e.g. `com.learning.kafka.consumer.AvroConsumer`).

**Why `OrderEventAvro` stays at the root instead of `dto/`**: same reason
as `producer-demo` — it's generated from the schema's `namespace` field,
which becomes the fully-qualified class name `AvroConsumer` resolves via
`specific.avro.reader=true` (`Class.forName(namespace + "." + name)`, read
from the *writer's* schema). Moving it here wouldn't break this project's
own consumer, but `producer-demo`'s `AvroProducer` still writes messages
tagged with `com.learning.kafka.OrderEventAvro` — changing the namespace
in just this project would create a mismatch, so it stays put everywhere
consistently.
